    ...
    def deploy(self) -> bool:
        ...
        try:
            ...
            image, build_logs = client.images.build(
                path=self.path, tag=f"evodev-web/{self.name}:latest", rm=True
            )
            ...
            # Uruchom kontener
            logger.info(
                f"Uruchamianie kontenera {self.container_name} na porcie {self.port}..."
            )
            ...
            logger.info(
                f"Projekt {self.name} został wdrożony jako kontener {self.container_name}"
            )
...
def create_and_deploy_project(
    name: str, project_type: str, port: int = 8088
) -> Tuple[bool, str]:
    ...
    try:
        ...
        return (
            True,
            f"Projekt {name} został utworzony i wdrożony pod adresem http://localhost:{port}",
        )
