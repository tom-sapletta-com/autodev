...
try:
    import evodev.web_projects as web_projects
except ImportError:
...
class EvoDev:
    ...
    def process_command(self, command: str) -> str:
        ...
        create_web_match = re.search(
            r"(stwórz|utwórz|zrób|wykonaj)\s+stron[ęea]\s+testow[ąa]", command.lower()
        )
        if create_web_match:
    ...
    def _handle_run_web_command(self, command: str) -> str:
        ...
        if matching_project:
            ...
            if project and project.start():
                return (
                    f"Uruchomiłem stronę {matching_project['name']} na porcie {port}. "
                    f"Dostępna pod adresem http://localhost:{port}"
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EvoDev - system do automatyzacji procesów deweloperskich"
    )
